        ...
        <section class="py-2">
            <div class="w-full max-w-7xl mx-auto px-6 md:px-8">
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 lg:gap-0">
                    <div class="block">
                        <h2 class="font-manrope font-bold text-2xl leading-9 text-gray-900 mb-3">Update Event</h2>
...
@endphp --}}
        <section>
            <div>
                <form class="max-w-md mx-auto" method="POST" action="{{ route('update_event', $event->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')

                    <div class="grid md:grid-cols-2 md:gap-6">
                        <div class="relative z-0 w-full mb-5 group">
                            <input type="text" name="name" id="floating_first_name" value="{{ $event->name }}"
                        ...
                        <div class="relative z-0 w-full mb-5 group">
                            <input type="text" name="location" id="floating_last_name" value="{{ $event->location }}"
                                class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                                placeholder=" " required />
                    ...
                    <div class="col-md-6 flex flex-col  pb-3">
                        Description
                        <textarea class="input-styling" name="description" id="" cols="30" rows="3">{{ $event->description }}</textarea>
                    </div>
                            ...
                            <select id="category" name="category" class="form-select">
                                ...
                                @foreach ($categories as $category)
                                    <option value="{{ $category }}"
                                        {{ $event->category == $category ? 'selected' : '' }}>{{ $category }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
